...
class A2CBuilder(NetworkBuilder):
    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            #         'input_size': 
            #     }
            mlp_input_shapes = [rnn_in_size+in_mlp_shape]
            for i in range(1, self.mlp_num_modules):
                mlp_input_shapes.append(self.units[-1] + mlp_input_shapes[i-1])
            mlp_modules = list()
            for i in range(self.mlp_num_modules - 1):
                mlp_args = {
                    'input_size' : mlp_input_shapes[i], 
                    'units' : self.units, 
            ...
            if self.separate:
                ...
                if self.has_rnn:
                    ...
                    if self.is_rnn_before_mlp:
                        ...
                        for i in range(self.mlp_num_modules - 1):
                            out = self.mlp_modules[i](prev_input)
                            prev_input = torch.cat([out, prev_input], dim=-1)
                        out = self.actor_mlp(prev_input)
