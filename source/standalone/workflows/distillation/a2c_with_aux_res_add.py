    ...
    class Network(NetworkBuilder.BaseNetwork):
        def __init__(self, params, **kwargs):
            ...
            if self.has_rnn:
                ...
                if self.is_aux:
                    mlp_args = {
                        'input_size': self.rnn_units + input_shape[0],
            ...
            # for i in self.mlp_num_modules - 1:
            #     mlp_args = {
            #         'input_size': 
            #     }

            self.rnn_to_mlp = nn.Sequential(
                nn.Linear(self.rnn_units+input_shape[0], self.units[0]),
                self.activations_factory.create(self.activation)
            )
            mlp_modules = list()
            for i in range(self.mlp_num_modules):
                mlp_args = {
                ...
                mlp_modules.append(self._build_mlp(**mlp_args))
                
            self.mlp_modules = nn.ModuleList(mlp_modules)
        ...
        def forward(self, obs_dict):
            ...
            if self.separate:
                ...
                if self.has_rnn:
                    ...
                    if self.is_rnn_before_mlp:
                        rnn_out = out
                        prev_input = torch.cat([out, concatenated_input], dim=-1)
                        prev_input = self.rnn_to_mlp(prev_input)
                        out = self.mlp_modules[0](prev_input)
                        for mlp_module in self.mlp_modules[1:]:
                            curr_input = prev_input+out
                            out = mlp_module(curr_input)
                            prev_input = curr_input
                        out = self.actor_mlp(prev_input+out)
        ...
        def load(self, params):
            ...
            self.mlp_num_modules = params['mlp']['num_modules']
