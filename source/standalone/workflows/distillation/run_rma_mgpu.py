...
from distillation_rma import Dagger

from source.standalone.workflows.rl_games.a2c_rma import A2CBuilder as A2CWithRMABuilderTeacher
from a2c_rma_student import A2CBuilder as A2CWithRMABuilderStudent
...
def main():
    ...
    student_cfg = os.path.join(
        parent_path,
        agent_cfg_folder,
        "rl_games_ppo_student_rma.yaml"
    )
    teacher_cfg = os.path.join(
        parent_path,
        agent_cfg_folder,
        "rl_games_ppo_teacher_rma.yaml"
    )
    ...
    if rank == 0:
        train_dir = "runs"
        experiment_name = (
            "Shadow-Hand-Camera-RMA-Distillation-MGPU"
